...
export class NotificationService {
  ...
  // Send notifications to devices (handles both iOS and Android)
  async sendToDevices(tokens, notification) {
    ...
    if (androidTokens.length > 0) {
      try {
        const fcmResult = await sendBatchFCMNotifications(androidTokens, notification, notification.data);
    ...
    if (iosTokens.length > 0) {
      try {
        const apnsResult = await sendBatchAPNSNotifications(iosTokens, notification, notification.data);
  ...
  async sendTestNotification(userId, deviceToken, platform) {
    try {
      ...
      console.log(`✅ Test notification sent to ${platform} device`);
      return result;
