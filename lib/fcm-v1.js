...
export async function sendFCMv1Notification(deviceToken, notification, data = {}) {
  try {
    ...
    if (!notification.title || !notification.body) {
      console.error('❌ Missing notification title or body:', { title: notification.title, body: notification.body });
    }
    ...
    const message = {
      token: deviceToken,
      notification: {
        title: notification.title || 'PartySnap',
    ...
    console.log('📦 FCM V1 Message being sent:');
    console.log('  Token:', deviceToken.substring(0, 20) + '...');
    console.log('  Channel ID:', getChannelId(notification.type));
...
export async function sendBatchFCMv1Notifications(tokens, notification, data = {}) {
  ...
  console.log('📦 FCM V1 Batch - Preparing to send:');
  console.log('  Tokens count:', validTokens.length);
  console.log('  Channel ID:', getChannelId(notification.type));
  ...
  if (messages.length > 0) {
    console.log('  Sample message payload:', JSON.stringify(messages[0], null, 2));
  }
