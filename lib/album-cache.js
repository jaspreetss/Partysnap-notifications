...
async function batchGenerateSignedUrls(photoPaths, options = {}) {
  ...
  try {
    ...
    if (urlsToGenerate.length > 0) {
      ...
      if (isAnonymousUser) {
        console.log('🔓 Anonymous user detected for album, creating supabase client with anon key');
        ...
        const urlPromises = urlsToGenerate.map(async (path) => {
          try {
            ...
            if (error || !data?.signedUrl) {
              console.error(`Failed to generate anonymous album URL for ${path}:`, error);
              return { path, url: null };
            }

            return { path, url: data.signedUrl };
          } catch (error) {
            console.error(`Exception generating anonymous album URL for ${path}:`, error);
            return { path, url: null };
          }
        });
        ...
        console.log(`🔗 Generated ${Object.keys(generatedUrls).length} anonymous album signed URLs`);
      } else {
        console.log('🔒 Authenticated user for album, generating signed URLs with service role');
        
        // Batch generate signed URLs using Supabase service role (for authenticated users)
        const urlPromises = urlsToGenerate.map(async (path) => {
          try {
