  ...
  try {
    ...
    if (!forceRefresh) {
      ...
      if (cached && !cached.is_expired) {
        ...
        // Check if any cached URLs are expired
        let hasExpiredUrls = false;
        if (cached.albums && cached.albums.length > 0) {
          for (const album of cached.albums) {
            if (album.processed_photos && album.processed_photos.length > 0) {
              for (const photo of album.processed_photos) {
                if (photo.photo_url && needsUrlConversion(photo.photo_url)) {
                  hasExpiredUrls = true;
                  console.log(`⚠️ Found expired URL in cached album data, will refresh`);
                  break;
                }
              }
              if (hasExpiredUrls) break;
            }
          }
        }
        
        // If URLs are expired, force a refresh
        if (hasExpiredUrls) {
          console.log(`🔄 Cached album has expired URLs, refreshing from database`);
        } else {
          return {
            ...cached,
            cache_hit: true,
...
function needsUrlConversion(url) {
  ...
  // Check if signed URL is expired (Supabase uses JWT tokens)
  if (url.includes('token=')) {
    try {
      ...
      if (token) {
        ...
        if (parts.length === 3) {
          ...
          if (payload.exp) {
            ...
            const buffer = 300; // 5 minute buffer in seconds
            const isExpired = now + buffer >= payload.exp;
            
            if (isExpired) {
              console.log(`🔄 Detected expired Supabase URL token (exp: ${payload.exp}, now: ${now})`);
            }
      ...
      console.warn('Error checking Supabase URL token expiry:', error);
