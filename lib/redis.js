...
async function upstashRestCall(command, ...args) {
  try {
    // Debug logging (only log presence of env vars, not actual values)
    if (!process.env.UPSTASH_REDIS_REST_URL) {
      throw new Error('UPSTASH_REDIS_REST_URL environment variable not set');
    }
    
    if (!process.env.UPSTASH_REDIS_REST_TOKEN) {
      throw new Error('UPSTASH_REDIS_REST_TOKEN environment variable not set');
    }

    console.log(`🔄 Upstash REST API call: ${command}`, args.length > 0 ? `(${args.length} args)` : '');
    ...
    if (!response.ok) {
      ...
      console.error(`Upstash API error details:`, {
    ...
    console.log(`✅ Upstash REST API success: ${command}`);
    return data.result;
  } catch (error) {
    console.error('Upstash REST API error:', {
      command,
      error: error.message,
      hasUrl: !!process.env.UPSTASH_REDIS_REST_URL,
      hasToken: !!process.env.UPSTASH_REDIS_REST_TOKEN,
      urlPattern: process.env.UPSTASH_REDIS_REST_URL ? 
        process.env.UPSTASH_REDIS_REST_URL.substring(0, 30) + '...' : 'undefined'
    });
    throw error;
  }
}
