...
async function generateSignedUrlsBatch(photoPaths, expiresIn, userId) {
  ...
  try {
    // Check if this is for an anonymous user
    const isAnonymousUser = !userId || userId.startsWith('anon_');
    
    if (isAnonymousUser) {
      console.log('🔓 Anonymous user detected, creating supabase client with anon key for signed URLs');
      
      // Import createClient for anonymous user signed URLs
      const { createClient } = await import('@supabase/supabase-js');
      
      // Create anonymous client for generating URLs that anonymous users can access
      ...
      // Generate URLs with anonymous client
      const batchSize = 20;
      for (let i = 0; i < photoPaths.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (path) => {
          try {
            ...
            if (error) {
              console.error(`Error generating anonymous signed URL for ${path}:`, error);
              return { path, url: null, error: error.message };
            }

            if (!data?.signedUrl) {
              console.error(`No signed URL returned for ${path} (anonymous)`);
              return { path, url: null, error: 'No URL returned' };
            }

            return { path, url: data.signedUrl, error: null };
          } catch (error) {
            console.error(`Exception generating anonymous signed URL for ${path}:`, error);
            return { path, url: null, error: error.message };
          }
        });
        ...
        batchResults.forEach((result, index) => {
          if (result.status === 'fulfilled' && result.value.url) {
            ...
            console.error(`Failed to generate anonymous URL for ${path}:`, 
              result.status === 'fulfilled' ? result.value.error : result.reason);
          }
        });
      }

      console.log(`🔗 Generated ${Object.keys(urls).length}/${photoPaths.length} anonymous signed URLs`);
      return urls;
    }

    // For authenticated users, use service role as before
    console.log('🔒 Authenticated user, generating signed URLs with service role');
