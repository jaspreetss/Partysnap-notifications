...
export async function shouldSendNotification(userId) {
  try {
    // TEMPORARY: Skip preference check for testing
    // TODO: Fix the RPC function or ensure user preferences are set
    console.log('⚠️ Bypassing notification preference check for testing');
    return true;
...
export async function storePushToken(userId, token, platform, deviceId, metadata = {}) {
  try {
    ...
    // Store metadata separately in memory/cache for now
    // This will be used by FCM V1 to determine token type
    if (metadata.tokenType) {
      console.log(`Token registered as ${metadata.tokenType}:`, token.substring(0, 20) + '...');
    }
